...
#define FOCUS_LEVEL_NORMAL 0
#define FOCUS_LEVEL_ONEX 1
#define FOCUS_LEVEL_TWOX 2
#define FOCUS_LEVEL_THRX 3
#define FOCUS_LEVEL_FOURX 4
#define FOCUS_LEVEL_FILEX 5
...
extern void ipu_driver_update_test();
static int preview_stop_flag ;
static int cim_fb_alloc_ext(void);
static void ipu_restart_preview();
struct cim_device
{
...
{
	if(desc==NULL){
		printk("sensor register desc is NULL\n");
		return -EINVAL;
	}
...
static int cim_fb_alloc_ext(void)
{
	...
	if (jz_cim->mem_base == 0 )
	{
		...
		jz_cim->mem_size = mem_size;
		jz_cim->ginfo.mmap_size = mem_size;
		printk("jz cim preview info vaddr = 0x%x paddr = 0x%x mem_size = 0x%x\n",jz_cim->preview_binfo.base_vaddr,jz_cim->preview_binfo.base_paddr,mem_size);
	}
...
static unsigned int gl_offset;
unsigned int cim_read_dma()
{
#if 0
	gl_offset=(unsigned int)(frame_desc[SWAP_BUF-1].framebuf);
	//gl_offset = cim_get_preview_buf(1);
	return gl_offset;
#else
	unsigned int tmp_paddr;
	tmp_paddr = cim_get_preview_buf(1);
	if (tmp_paddr != (unsigned int)(~0)){
		gl_offset = tmp_paddr;
	}else{
	}
	//return (gl_offset-jz_cim->preview_binfo.base_paddr);
	return (gl_offset);
#endif
}
EXPORT_SYMBOL(cim_read_dma);
 ...
 *     IOCTL Handlers     *
 **************************/
extern void ipu_driver_stop_new();
extern void ipu_driver_init_test(int,int,int,int);
static int old_tv_flag = 0;
...
#if 0 //allen del
	ipu_driver_stop_new();
	extern int get_tvout_flag();
	int ret = get_tvout_flag();
	if (ret == old_tv_flag)
		return;

	printk("preview restar to tvout!");
	old_tv_flag = ret;

	if (get_tvout_flag()){
		gl_dst_w = 640;
		gl_dst_h = 480;
		ipu_driver_init_test(cur_desc->preview_parm.width,cur_desc->preview_parm.height,gl_dst_w,gl_dst_h);
	}else{
		gl_dst_w = get_lcd_width(); //480;
		gl_dst_h = get_lcd_hight(); //320;
		ipu_driver_init_test(cur_desc->preview_parm.width,cur_desc->preview_parm.height,gl_dst_w,gl_dst_h);
	}
#endif

}
extern void ipu_driver_set_src_stride(int src_w, int dst_w);
static int cim_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_CIM_CLOSE_TV:
		...
		case IOCTL_CIM_START_PREVIEW:
			{
				int focus_level = 0;
				int preview_tmp_width,preview_tmp_height;
				if (copy_from_user(&focus_level, (void *)arg, sizeof(focus_level)))
					return -EFAULT;
				//printk("focus level is %d\n\n",focus_level);
				//dprintk("IOCTL_CIM_START_PREVIEW\n");
				cim_start_preview();
				//dprintk("after IOCTL_CIM_START_PREVIEW\n");
				switch (focus_level){
					case FOCUS_LEVEL_NORMAL:
						preview_tmp_width = cur_desc->preview_parm.width;
						preview_tmp_height = cur_desc->preview_parm.height;
						break;
					case FOCUS_LEVEL_ONEX:
						preview_tmp_width = 560;
						preview_tmp_height = 416;
						break;
					case FOCUS_LEVEL_TWOX:
						preview_tmp_width = 480;
						preview_tmp_height = 352;
						break;
					case FOCUS_LEVEL_THRX:
						preview_tmp_width = 400;
						preview_tmp_height = 288;
						break;
					case FOCUS_LEVEL_FOURX:
						preview_tmp_width = 320;
						preview_tmp_height = 224;
						break;
#if 0
					case FOCUS_LEVEL_FILEX:
						preview_tmp_width = 240;
						preview_tmp_height = 160;
						break;
#endif
					default:
						preview_tmp_width = cur_desc->preview_parm.width;
				...
				//no support tvout!
				ipu_driver_set_src_stride(cur_desc->preview_parm.width,gl_dst_w);
				ipu_driver_init_test(preview_tmp_width,preview_tmp_height,gl_dst_w,(gl_dst_h+4));
				return 0;
			}
	...
	case IOCTL_CIM_GET_PREVIEW_OFFSET:
	{
		...
		else
		{
			...
			//if(gl_offset == (unsigned int)(~0))
			//	return -1;
			//gl_offset -= jz_cim->preview_binfo.base_paddr;
			offset = gl_offset - jz_cim->preview_binfo.base_paddr;
		}
